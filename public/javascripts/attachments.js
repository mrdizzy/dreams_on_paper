...
// A CouchDB model is a model that has an _attachments field, 
// a _rev and an _id field. We start by extending the standard
// Backbone.Model class.
// _attachments is an instance of Backbone.Collection.Attachments
// and we set a parent attribute on that collection instance
// which points back to this particular model
...
Backbone.Model.CouchDB = Backbone.Model.extend({
      ...
      var number = parseInt(number) 
        ...
        if(resp._attachments[id]) {
            ...
            var attachment = {
                ...
                size: (resp._attachments[id].length/1000)
            }
  ...
  // save our model to the server. We start by cloning the current attributes
  // into a separate object and then adding an empty _attachments attribute
  // to that object. We also copy across the attachments_order array to that
  // object. We then loop through the attachments checking to see if it is
  // a stub or not, and if it is not a stub we add the data by its id.
  toJSON: function() {
...
Backbone.View.CouchDB = Backbone.View.extend({
  ...
  // from within the extended view's render() method, passing it a Backbone view
